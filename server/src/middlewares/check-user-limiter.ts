        ...
        if (user.stripe_account_id) {
            try {
                ...
                if (planMetadata?.max_link) {
                    ...
                    maxLinks = getPlanLinkLimitByPrice(priceAmount);
                }
...
export const check_pages_limiter_status = async (
    req: any,
    res: Response,
    next: NextFunction
) => {
    ...
    try {
        ...
        if(user.stripe_account_id){
            try {
                ...
                // Get limit from Stripe product metadata
                if (planMetadata?.max_page) {
                    maxPage = parseInt(planMetadata.max_page);
                } else {
                    // Fallback: determine by price
                    const priceAmount = subscription.items.data[0]?.price?.unit_amount || 0;
                    maxPage = getPlanPageLimitByPrice(priceAmount);
                }
        ...
        if (pageCount >= maxPage) {
            return res.status(StatusCodes.PAYMENT_REQUIRED).json({
                ...
                current: pageCount,
                limit: maxPage,
                plan: getPlanNameByLimit(maxPage),
                upgradeRequired: true
            });
        }
        ...
        req.pageLimits = {
            current: pageCount,
            limit: maxPage,
            remaining: maxPage - pageCount
        };

        next();
    } catch (error) {
        console.error("Page limiter error:", error);
        // On error, allow creation but log the issue
        next();
    }
};
...
const checkUserSubscription = async (stripe_account_id: string) => {
    ...
    if (subscriptions.data.length > 0) {
        ...
        return [planMetadata, subscription];
    }
...
const getPlanLinkLimitByPrice = (priceAmount: number): number => {
    const priceInDollars = priceAmount / 100;

    if (priceInDollars === 0) return 5;      // Free
    if (priceInDollars === 10) return 500;   // Pro
    if (priceInDollars === 20) return 1000;  // Premium

    return 5; // Default to free tier
};

const getPlanPageLimitByPrice = (priceAmount: number) => {
    const priceInDollars = priceAmount / 100;

    if (priceInDollars === 0) return 2;      // Free
    if (priceInDollars === 10) return 50;   // Pro
    if (priceInDollars === 20) return 100;  // Premium

    return 2; // Default to free tier
};
